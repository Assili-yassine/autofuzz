...
- **Install succeeds, `autofuzz: command not found` afterward:** pip printed
  a warning during install like
  `The script autofuzz is installed in '/root/.local/bin' which is not on PATH`
  — the install worked, your shell just can't find that directory. Fix:
  ```bash
  export PATH="$HOME/.local/bin:$PATH"      # this shell
  echo 'export PATH="$HOME/.local/bin:$PATH"' >> ~/.bashrc   # permanently
  ```
  If you're root and used `pip install --user .` directly (bypassing
  `install.sh`), the simplest fix is often to just install system-wide
  instead, which lands in `/usr/local/bin` (already on PATH for everyone):
  ```bash
