...
def run_pipeline_for_target(
    ...
    (target_dir / "secret.txt").write_text(
        "\n".join(f"{s['confidence']}\t{s['type']}\t{s['match']}\t{s['source']}" for s in result.secrets),
        encoding="utf-8",
    )
    elapsed = time.time() - t0
    high_conf = sum(1 for s in result.secrets if s.get("confidence") == "high")
    reporter.stage_done(url, 5, elapsed, detail=f"{len(result.secrets)} hits ({high_conf} high-confidence)")
